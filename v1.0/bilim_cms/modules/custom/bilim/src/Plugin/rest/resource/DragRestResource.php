 ...
 *   uri_paths = {
 *     "canonical" = "/api/change/elem/{entity_id}"
 *   }
 * )
 */
class DragRestResource extends ResourceBase {
  ...
  */
   ...
   */
    public function patch($param,$data) {
        ...
        $tnode = Node::load($tid);
        $ttype = $tnode->getType();
        if($ttype == 'feedback' || $ttype == 'partpage'):
          ...
          if($cur_parent == $new_parent):
            ...
            $new_swapped_children = $bcr->swapElem($chd_array,$new_pos,$tid);
