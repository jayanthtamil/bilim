...
const BlockTree = {
   ...
   */
  generate(contentState: ContentState, block: BlockNodeRecord, decorator // $FlowFixMe[value-as-type]
  : ?DraftDecoratorType): List<DecoratorRange> {
...
function generateLeaves(characters: List<CharacterMetadata>, offset // $FlowFixMe[value-as-type]
: number): List<LeafRange> {
  const leaves = [];
