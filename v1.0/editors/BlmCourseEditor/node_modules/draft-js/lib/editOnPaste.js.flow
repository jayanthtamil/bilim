  ...
  if (!editor.props.stripPastedStyles) {
    ...
    if (!editor.props.formatPastedText && data.isRichText() && internalClipboard) {
      ...
      textBlocks.length === 1 && internalClipboard.size === 1 && internalClipboard.first().getText() === text) {
        handleInternalPaste = () => editor.update(insertFragment(editor._latestEditorState, internalClipboard));
      }
...
function insertFragment(editorState: EditorState, fragment: BlockMap, entityMap: ?EntityMap): EditorState {
  const newContent = DraftModifier.replaceWithFragment(editorState.getCurrentContent(), editorState.getSelection(), fragment);
  const newEntityMap = entityMap ? ContentState.mergeEntityMaps(newContent.getAllEntities(), entityMap) : newContent.getAllEntities();
