...
interface CheckInfo {
  ...
  checkedNodesPositions?: { node: NodeInstance; pos: string }[];
  halfCheckedKeys?: Key[];
}
...
class Tree extends React.Component<TreeProps, TreeState> {
  ...
  onNodeDragStart = (event, node) => {
    const { dropPosition, expandedKeys } = this.state;
  ...
  onNodeDragOver = (event, node) => {
    ...
    if (onDragOver) {
      onDragOver({ event, node, dragNode: this.dragNode, dropPosition });
    }
  };
  ...
  onNodeDragLeave = (event, node) => {
    ...
    if (onDragLeave) {
      onDragLeave({ event, node, dragNode: this.dragNode, dropPosition });
    }
  };
  ...
  onNodeDragEnd = (event, node) => {
    ...
    if (onDragEnd) {
      onDragEnd({ event, node, dragNode: this.dragNode, dropPosition });
    }
  ...
  onNodeExpand = (e, treeNode) => {
    ...
    const { eventKey, expanded, alwaysExpanded } = treeNode.props;
  ...
  onNodeOptionsClick = (event, node) => {
    const { onOptionsClick } = this.props;
    if (onOptionsClick) {
      event.preventDefault();
      onOptionsClick({ event, node });
    }
  };
  /**
  ...
  render() {
    ...
    const domProps: React.HTMLAttributes<HTMLUListElement> = getDataAndAria(
      this.props
    );
