...
class TreeNode extends React.Component<InternalTreeNodeProps, TreeNodeState> {
  ...
  onSelectorClick = (e) => {
    ...
    const { expandOnSelectorClick } = this.props;
    const {
      context: {
        onNodeClick,
        expandOnSelectorClick: treeExpandOnSelectorClick,
      },
    } = this.props;
    onNodeClick(e, this);

    const expandOnSelector =
      typeof expandOnSelectorClick !== "undefined"
        ? expandOnSelectorClick
        : treeExpandOnSelectorClick;
  ...
  onDragOver = (e) => {
    const {
      context: { onNodeDragOver, getNodeDropEffect },
    } = this.props;
    ...
    if (getNodeDropEffect) {
      const dropEffect = getNodeDropEffect(e, this);

      if (this.state.dropEffect !== dropEffect) {
        this.setState({
          dropEffect,
        });
      }
  ...
  onOptionsClick = (e) => {
    const {
      context: { onNodeOptionsClick },
    } = this.props;
    onNodeOptionsClick(e, this);
  };
  ...
  getNodeExpanded = () => {
    const { expanded, alwaysExpanded } = this.props;

    return alwaysExpanded || expanded;
  };

  getNodeState = () => {
    const expanded = this.getNodeExpanded();
  ...
  //Options
  renderOptionsIcon = () => {
    const { showOptionsIcon, optionsIcon } = this.props;
    const {
      context: { prefixCls, showOptionsIcon: treeShowOptionsIcon },
    } = this.props;

    let $icon;
    const showOptions =
      typeof showOptionsIcon !== "undefined"
        ? showOptionsIcon
        : treeShowOptionsIcon;
    ...
    if (showOptions) {
      $icon = (
        <span
          className={classNames(`${prefixCls}-options`)}
    ...
    return (
      <li
        className={classNames(className, {
          ...
          "drag-over":
            !disabled && dragOver && this.state.dropEffect !== "none",
          "drag-over-gap-top":
            !disabled && dragOverGapTop && this.state.dropEffect !== "none",
          "drag-over-gap-bottom":
            !disabled && dragOverGapBottom && this.state.dropEffect !== "none",
      ...
      >
        <div className={classNames(`${prefixCls}-node-row`)}>
          <div className={classNames(`${prefixCls}-node-row-container`)}>
            {this.renderSwitcher()}
            {this.renderCheckbox()}
            {this.renderSelector()}
          </div>
