...
describe("Tree Basic", () => {
  ...
  describe("check", () => {
    ...
    it("ignore disabled children when calculate parent's checked status", () => {
      ...
      const firstCheckboxClassNames = wrapper
        .find(".rc-tree-checkbox")
        .instance().classList;
      expect(
        [].slice
          .call(firstCheckboxClassNames)
          .includes(CHECKED_CLASSNAME.slice(1))
      ).toBe(false);
    });
    ...
    it("should ignore disableCheckbox children items when check parent", () => {
      ...
      expect(firstNode().hasClass("rc-tree-checkbox-indeterminate")).toBe(
        false
      );
      expect(
        wrapper
    ...
    it("should render parent check state correctly", () => {
      ...
      expect(getParent().hasClass("rc-tree-checkbox-indeterminate")).toBe(
        false
      );
  ...
  it("fires rightClick event", () => {
    ...
    expect(handleRightClick.mock.calls[0][0].node).toBe(
      wrapper.find(InternalTreeNode).instance()
    );
  });
  ...
  it("fires mouseEnter events", () => {
    ...
    expect(handleMouseEnter.mock.calls[0][0].node).toBe(
      wrapper.find(InternalTreeNode).instance()
    );
  });
  ...
  it("fires mouseLeave events", () => {
    ...
    expect(handleMouseLeave.mock.calls[0][0].node).toBe(
      wrapper.find(InternalTreeNode).instance()
    );
  });
  ...
  describe("drag and drop", () => {
    ...
    it("fires dragStart event", () => {
      ...
      const treeNode = wrapper.find(
        ".dragTarget > .rc-tree-node-row > .rc-tree-node-row-container > .rc-tree-node-content-wrapper"
      );
