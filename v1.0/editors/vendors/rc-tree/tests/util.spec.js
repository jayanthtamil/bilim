...
describe("Util", () => {
  ...
  it("convertTreeToEntities with additional handler", () => {
    ...
    const { keyEntities, valueEntities } = convertTreeToEntities(
      tree.props.children,
      {
        initWrapper: wrapper => ({
          ...wrapper,
  ...
  describe("conductCheck", () => {
    describe("basic", () => {
      ...
      it("not exist", () => {
        console.log(
          ">>> Follow Warning is for test purpose. Don't be scared :)"
        );
        const tree = genTree();
        const { keyEntities } = convertTreeToEntities(tree.props.children);
        const { checkedKeys, halfCheckedKeys } = conductCheck(
    ...
    describe("part disabled", () => {
      ...
      it("uncheck", () => {
        ...
        // Then uncheck one of then
        const result1 = conductCheck(["not"], false, keyEntities, {
          checkedKeys: allCheckedKeys
        });
